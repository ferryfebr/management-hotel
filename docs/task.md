...
- [x] **Cek ulang semua middleware role** di `routes/web.php` — terverifikasi: owner-only (`/dashboard`, `/reports`, `/room-types`, destroy customers/rooms) terpisah; owner+resepsionis untuk kamar/pelanggan/reservasi/transaksi; room_keeper khusus `/room-keeper`. Route `DeployController` baru DI LUAR grup auth & terproteksi token.
...
- [x] **Siapkan `DeployController`** untuk migration tanpa SSH (lihat `structure.md` §4.2 Cara B) — `POST /deploy/{token}`, token dari env `DEPLOY_TOKEN`, `hash_equals` anti timing-attack, opsi `?seed=1` untuk `ProductionSeeder`. ⚠️ **HAPUS route+controller ini setelah dipakai sekali** (lihat komentar di `routes/web.php`).
- [~] **Set `.env` production**: `APP_DEBUG=false`, `APP_ENV=production`, isi kredensial DB production — **belum selesai**, menunggu Fase 3 (beli hosting & buat DB). Template placeholder `DEPLOY_TOKEN` & `PROD_*` sudah ada di `.env.example`.
- [x] **Cek folder upload foto KTP** — foto KTP disimpan di disk `private` (`storage/app/private`, di luar `public/`), jadi tidak bisa kena *directory-listing* publik. `.htaccess` tidak diperlukan di sini; tetap verifikasi document root mengarah ke `public/` saat deploy (Fase 4).
